...
{
	if(argc < 3){
		printf("usage:ssl_server 172.24.10.113 443\r\n");
		return 0;
	}
    int ret, len;
...
#define LJG_TEST_MODE

#ifdef LJG_TEST_MODE
#ifdef _WIN32
    ret = mbedtls_x509_crt_parse_file(&srvcert, "d:\\demo_tls_cert.pem");
    ret = mbedtls_pk_parse_keyfile(&pkey, "d:\\demo_tls_key.pem", "");
    printf("\r\nwindows platform\r\n");
#elif defined __linux__
    ret = mbedtls_x509_crt_parse_file(&srvcert, "./demo_tls_cert.pem");
    ret = mbedtls_pk_parse_keyfile(&pkey, "./demo_tls_key.pem", "");
    printf("\r\nlinux platform\r\n");
#else
    ...
    if ((ret = mbedtls_net_bind(&listen_fd, srvip,srvport , MBEDTLS_NET_PROTO_TCP)) != 0) {
