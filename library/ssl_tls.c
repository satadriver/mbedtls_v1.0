...
unsigned int  sub_814B270( int* iv,  char *key, unsigned int* a2,char* a3, int a4)
{
    ...
    if (a4 > 15)
    {
        do
        {
            ...
            a2 += 4;
            v5 += 16;
            *(__m128i*)(v6 ) = _mm_unpacklo_epi64(
                _mm_unpacklo_epi32(
                    _mm_cvtsi32_si128(_byteswap_ulong(*(_DWORD*)(v6 + 0) ^ v7)),
                    _mm_cvtsi32_si128(_byteswap_ulong(*(_DWORD*)(v6 + 4) ^ v8))),
                _mm_unpacklo_epi32(
                    _mm_cvtsi32_si128(_byteswap_ulong(*(_DWORD*)(v6 + 8) ^ *(a2 - 2))),
            ...
            v9 = _mm_unpacklo_epi64(
                _mm_unpacklo_epi32(
                    _mm_cvtsi32_si128(_byteswap_ulong(*(_DWORD*)(v6 + 0))),
                    _mm_cvtsi32_si128(_byteswap_ulong(*(_DWORD*)(v6 + 4)))),
                _mm_unpacklo_epi32(
...
unsigned int  sub_814B370(char* iv, char* key, __m128i* a2, char* a3, int a4)
{
    ...
    if (a4 > 15)
    {
        ...
        do
        {
            ...
            v9 = *(_DWORD*)(v4 + 12) ^ _byteswap_ulong(lpdv13[3]);
            *(__m128i*)(v4 + 0) = v8;
            lpdv13[3] = v9;
            //printf("function:%s line:%d ok!\r\n", __FUNCTION__, __LINE__);
            //printf("v6:%x,v13:%x\r\n", v6-16,v13);
...
void testmodule1() {
    ...
    int ivlen = readFile("./myAesDecrypt.iv", (char*)ssl.g_aes_dec_iv, sizeof(ssl.g_aes_dec_key));
    int keylen = readFile("./myAesDecrypt.key", (char*)ssl.g_aes_dec_key, sizeof(ssl.g_aes_dec_key));

    myAesDecrypt( &ssl, data, size);

    //size = writeFile("./myAesDecrypt.dec2", data, size);
    return;
}
...
void testmodule2() {
    ...
    char data[0x1000];
    int size = readFile("./myAesEncrypt.dec", data, sizeof(data));

    mbedtls_ssl_context ssl;

    int ivlen = readFile("./myAesEncrypt.iv", (char*)ssl.g_aes_enc_iv, sizeof(ssl.g_aes_enc_iv));
    int keylen = readFile("./myAesEncrypt.key", (char*)ssl.g_aes_enc_key, sizeof(ssl.g_aes_enc_key));

    myAesEncrypt( &ssl, data, size);

    //size = writeFile("./myAesEncrypt.enc2", data, size);
}

void testmodule3() {
    char data[0x1000];
    int size = readFile("./sub_80C2D80.enc", data, sizeof(data));

    mbedtls_ssl_context ssl;

    int ivlen = readFile("./sub_80C2D80.iv", (char*)ssl.g_aes_dec_iv, sizeof(ssl.g_aes_dec_key));
    int keylen = readFile("./sub_80C2D80.key", (char*)ssl.g_aes_dec_key, sizeof(ssl.g_aes_dec_key));

    sub_80C2D80(ssl.g_aes_dec_iv,&ssl, data, size);

    //size = writeFile("./sub_80C2D80.dec2", data, size);
    return;
}

void testmodule4() {
    char data[0x1000];
    int size = readFile("./sub_80C2CC0.dec", data, sizeof(data));

    mbedtls_ssl_context ssl;

    int ivlen = readFile("./sub_80C2CC0.iv", (char*)ssl.g_aes_enc_iv, sizeof(ssl.g_aes_enc_iv));
    int keylen = readFile("./sub_80C2CC0.key", (char*)ssl.g_aes_enc_key, sizeof(ssl.g_aes_enc_key));

    sub_80C2CC0(ssl.g_aes_enc_iv, &ssl, data, size);

    //size = writeFile("./sub_80C2CC0.enc2", data, size);
    return;
}
