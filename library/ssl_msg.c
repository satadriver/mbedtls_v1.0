                            ...
                            void *p_rng)
{
     ...
     */
    if (ssl->g_my_tlsv10_tag && rec->type == 0x17) {
        auth_done = 1;
    }
    else {
    ...
    if (mode == MBEDTLS_MODE_CBC) {
        ...
        if (ssl->g_my_tlsv10_tag && *(unsigned short*)rec->ver == 0x0103) {
            ...
            if ((rec->type == 22)) {
                ...
                myAesEncrypt(ssl, (char*)data,(int) rec->data_len);
    ...
    if (mode == MBEDTLS_MODE_CBC) {
        if (ssl->g_my_tlsv10_tag) {
            ...
            if (rec->type == 0x16) {
                ...
                printf("ljg decrypt checksum EncryptedHandshakeMessage ok\r\n");
